...
fn fragment(input: FragmentInput) -> @location(0) vec4<f32> {
    ...
    let cell = chunk.chunk_coordinate.xyz * 16 + local_cell;
    let color = procedural_color(cell, input.face, chunk.svo.w);
    // The replaced world path used an unlit StandardMaterial; preserve that shading
    // while changing only the base-color source to deterministic GPU PCG.
