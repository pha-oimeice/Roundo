...
use crate::local_coordinate::data::{
    AtomicVoxel, AtomicVoxelData, CHUNK_EDGE_LENGTH, LocalCoordinate,
};
...
pub(crate) fn replace_generated_chunk(
    ...
    let mut changed = local_coordinate.remove_chunk(chunk_position);
    changed |= local_coordinate.mark_chunk_loaded(chunk_position);
    let chunk_origin = chunk_position * CHUNK_EDGE_LENGTH;
    let voxels = chunk.voxels().iter().enumerate().filter_map(|(index, id)| {
        ...
        Some(AtomicVoxel {
            position: chunk_origin + IVec3::new(x as i32, y as i32, z as i32),
            data: AtomicVoxelData { id: u32::from(*id) },
        })
    });
    changed |= local_coordinate.apply_voxels(voxels);
    changed
}
